...
# Define the LightGBM Regressor model fitting and evaluation
def lightgbm_regression(X, y, n_estimators=100, learning_rate=0.1, max_depth=-1, random_state=42):
    # Split the dataset into training and testing sets
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=random_state)

    # Fit the LightGBM Regressor model
    ...
    print(f"LightGBM Regressor Model MSE: {lgbm_mse}")
    print(f"LightGBM Regressor Model R-squared: {lgbm_r2}")

    # Perform cross-validation
    cv_mse = cross_val_score(lgbm_model, X, y, cv=5, scoring='neg_mean_squared_error')
    cv_r2 = cross_val_score(lgbm_model, X, y, cv=5, scoring='r2')
...
# Fit the LightGBM Regressor model with the desired parameters
n_estimators = 100  # Number of boosting stages
learning_rate = 0.1  # Step size shrinkage
max_depth = -1  # Maximum depth of individual trees (unlimited depth by default)
random_state = 42  # Seed for reproducibility
lgbm_model = lightgbm_regression(X, y, n_estimators=n_estimators, learning_rate=learning_rate, max_depth=max_depth, random_state=random_state)
...
result = minimize(objective_function, initial_guess, args=(lgbm_model), bounds=bounds, constraints=constraints, method='trust-constr')
