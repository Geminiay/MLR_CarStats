...
# Define the Lasso regression model fitting and evaluation
def lasso_regression(X, y, alpha=1.0):
    ...
    print(f"Lasso Regression Model R-squared: {lasso_r2}")

    # Perform cross-validation
    cv_mse = cross_val_score(lasso_model, X, y, cv=5, scoring='neg_mean_squared_error')
    cv_r2 = cross_val_score(lasso_model, X, y, cv=5, scoring='r2')
...
# Fit the Lasso regression model with the desired alpha value
alpha = 0.0002  # You can adjust this value for stronger or weaker regularization
lasso_model = lasso_regression(X, y, alpha=alpha)
...
result = minimize(objective_function, initial_guess, args=(lasso_model), bounds=bounds, constraints=constraints, method='trust-constr')
