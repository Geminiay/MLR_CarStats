...
# Define the Gradient Boosting Regressor model fitting and evaluation
def gbm_regression(X, y, n_estimators=100, learning_rate=0.1, max_depth=3, random_state=42):
    # Split the dataset into training and testing sets
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=random_state)

    # Fit the Gradient Boosting Regressor model
    gbm_model = GradientBoostingRegressor(n_estimators=n_estimators, learning_rate=learning_rate, max_depth=max_depth, random_state=random_state)
    ...
    print(f"Gradient Boosting Regressor Model MSE: {gbm_mse}")
    print(f"Gradient Boosting Regressor Model R-squared: {gbm_r2}")

    # Perform cross-validation
    cv_mse = cross_val_score(gbm_model, X, y, cv=5, scoring='neg_mean_squared_error')
    cv_r2 = cross_val_score(gbm_model, X, y, cv=5, scoring='r2')
...
# Fit the Gradient Boosting Regressor model with the desired parameters
n_estimators = 100  # Number of boosting stages
learning_rate = 0.1  # Step size shrinkage
max_depth = 3  # Maximum depth of individual trees
random_state = 42  # Seed for reproducibility
gbm_model = gbm_regression(X, y, n_estimators=n_estimators, learning_rate=learning_rate, max_depth=max_depth, random_state=random_state)
...
result = minimize(objective_function, initial_guess, args=(gbm_model), bounds=bounds, constraints=constraints, method='trust-constr')
