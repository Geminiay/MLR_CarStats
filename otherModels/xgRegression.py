...
# Define the XGBoost Regressor model fitting and evaluation
def xgboost_regression(X, y, n_estimators=100, learning_rate=0.1, max_depth=3, random_state=42):
    ...
    print(f"XGBoost Regressor Model MSE: {xgb_mse}")
    print(f"XGBoost Regressor Model R-squared: {xgb_r2}")

    # Perform cross-validation
    cv_mse = cross_val_score(xgb_model, X, y, cv=5, scoring='neg_mean_squared_error')
...
def objective_function(X_input, xgb_model):
...
# Fit the XGBoost Regressor model with the desired parameters
n_estimators = 100  # Number of boosting stages
learning_rate = 0.1  # Step size shrinkage
max_depth = 3  # Maximum depth of individual trees
random_state = 42  # Seed for reproducibility
xgb_model = xgboost_regression(X, y, n_estimators=n_estimators, learning_rate=learning_rate, max_depth=max_depth, random_state=random_state)
...
result = minimize(objective_function, initial_guess, args=(xgb_model), bounds=bounds, constraints=constraints, method='trust-constr')
